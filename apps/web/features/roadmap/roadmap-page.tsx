...
import { analysisAvailability, analysisGuideUrl, analysisReviewUrl } from "@/lib/analysis-capabilities";
...
function CapabilityGroup({ title, description, capabilities }: RoadmapPhase["capabilityGroups"][number]) {
  ...
  return <div className="atlas-capability-group"><h3 className="font-medium">{title}</h3>{description ? <p className="mt-1 text-sm leading-6 text-muted-foreground">{description}</p> : null}<ul className="mt-4 space-y-3">{capabilities.map((capability) => <CapabilityItem capability={capability} key={capability.name} />)}</ul></div>;
}
...
function RoadmapContent({ phases, source }: { phases: readonly RoadmapPhase[]; source: DataSource }) {
  ...
  return <PageContainer><Section><PageHeader actions={current ? <PhaseStatusBadge status={current.status} /> : undefined} description="Follow the progression from experiment infrastructure and grounded AI to causal analysis and decision intelligence." title="Roadmap" /><div className="mt-4 flex items-start gap-2 text-sm text-muted-foreground"><MapPinned aria-hidden="true" className="mt-0.5 size-4 shrink-0" /><p><span className="font-medium text-foreground">{source.label}.</span> {source.detail} Status is based on repository implementation and versioned project planning, not a live project-management integration.</p></div>{hasPartialData ? <div className="mt-6 rounded-lg border border-status-planned/40 bg-status-planned/10 p-4 text-sm" role="status"><p className="font-medium">Partial roadmap data</p><p className="mt-1 text-muted-foreground">{missingPhases.length ? `Missing phase metadata: ${missingPhases.join(", ")}. ` : ""}{duplicateNumbers ? "Duplicate phase numbers were returned. " : ""}{invalidActivePhase ? "The source defines more than one active phase." : ""}</p></div> : null}<section aria-labelledby="current-state" className="mt-8 rounded-xl border border-status-progress/50 bg-status-progress/10 p-5 sm:p-6"><h2 className="mt-3 text-2xl font-semibold tracking-tight" id="current-state">{current ? `${current.title} is in progress` : `${completed} phases implemented within documented scope`}</h2><div className="mt-5 grid gap-4 sm:grid-cols-3"><div><p className="text-2xl font-semibold">{completed}</p><p className="text-sm text-muted-foreground">phases implemented</p></div><div><p className="text-2xl font-semibold">{future}</p><p className="text-sm text-muted-foreground">future phases</p></div><div><p className="text-sm font-medium">Future direction</p><p className="mt-1 text-sm leading-5 text-muted-foreground">Enterprise platform capabilities and advanced research remain future work; no delivery dates are committed.</p></div></div></section><section className="mt-6 space-y-3 text-sm leading-6" aria-label="Analysis availability"><p>{analysisAvailability.interface}</p><p className="text-muted-foreground">{analysisAvailability.deployment}</p><div className="flex flex-wrap gap-x-5 gap-y-2"><a className="inline-flex min-h-11 items-center rounded-sm text-primary underline underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={analysisGuideUrl}>Supported methods and examples</a><a className="inline-flex min-h-11 items-center rounded-sm text-primary underline underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={analysisReviewUrl}>Phase 4 reliability review and advisories</a></div></section><section className="mt-10" aria-labelledby="roadmap-overview"><div className="flex items-center gap-2"><FlaskConical aria-hidden="true" className="size-4 text-primary" /><h2 className="text-xl font-semibold" id="roadmap-overview">Six-phase progression</h2></div><p className="mt-2 text-sm text-muted-foreground">Select a phase to jump to its evidence, technical role, and capability boundary.</p><div className="mt-5"><RoadmapTimeline phases={phases} /></div></section><section className="mt-10 space-y-5" aria-label="Detailed roadmap phases">{phases.map((phase) => <PhaseSection key={phase.id} phase={phase} />)}</section><section className="mt-10 grid gap-6 lg:grid-cols-2"><Card className="p-5"><h2 className="text-lg font-semibold">Status legend</h2><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="font-medium">Completed</dt><dd className="mt-1 text-muted-foreground">Implemented within the documented scope; this does not imply UI availability or production deployment.</dd></div><div><dt className="font-medium">In progress</dt><dd className="mt-1 text-muted-foreground">Active development focus, not a completed method.</dd></div><div><dt className="font-medium">Planned</dt><dd className="mt-1 text-muted-foreground">Named direction with no operational product result.</dd></div><div><dt className="font-medium">Future research</dt><dd className="mt-1 text-muted-foreground">Exploratory or future scope, not committed delivery.</dd></div></dl></Card><Card className="p-5"><h2 className="text-lg font-semibold">Portfolio relevance</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">Foundation demonstrates backend and data engineering; agent workflow demonstrates agentic architecture; LLMOps demonstrates reliability engineering; Product Intelligence demonstrates validated statistical and causal methods with explicit limitations; enterprise and research phases indicate the next system-design and decision-science directions.</p></Card></section><section className="mt-10 rounded-xl border bg-card/35 p-5 sm:p-6" aria-labelledby="technical-progression"><h2 className="text-lg font-semibold" id="technical-progression">Technical progression</h2><ol className="mt-5 space-y-2 text-sm"><li>Store experiment knowledge <ArrowDown aria-hidden="true" className="mx-2 inline size-3.5" /> Retrieve evidence <ArrowDown aria-hidden="true" className="mx-2 inline size-3.5" /> Generate grounded answers</li><li>Coordinate agent workflows <ArrowDown aria-hidden="true" className="mx-2 inline size-3.5" /> Evaluate AI reliability <ArrowDown aria-hidden="true" className="mx-2 inline size-3.5" /> Apply statistical analysis</li><li>Support decisions <ArrowDown aria-hidden="true" className="mx-2 inline size-3.5" /> Scale toward enterprise and research use</li></ol><p className="mt-5 flex gap-2 text-sm leading-6 text-muted-foreground"><CheckCircle2 aria-hidden="true" className="mt-0.5 size-4 shrink-0 text-status-completed" />The first four phases implement evidence retrieval, agent workflows, reliability checks, and bounded statistical analysis. Enterprise capabilities and advanced research remain future work.</p></section><p className="mt-10 max-w-3xl text-sm leading-6 text-muted-foreground">ExperimentOS AI evolves from experiment infrastructure into decision intelligence by keeping evidence, workflow controls, evaluation, and statistical claims explicit at every stage.</p></Section></PageContainer>;
}
