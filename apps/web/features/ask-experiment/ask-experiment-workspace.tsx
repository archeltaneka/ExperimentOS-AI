...
function QuestionWorkspace({ experimentId, source, samples, initialAnswer, contextSelector }: {
  ...
  return <div id="question-workspace" className="mt-8 grid scroll-mt-8 gap-6 xl:grid-cols-[minmax(19rem,0.78fr)_minmax(0,1.22fr)]">
    <ContentCard className="min-w-0 h-fit space-y-6 p-5 sm:p-6">
      ...
      {examples.length > 0 && <div><h3 className="text-sm font-medium">{isDemo ? "Supported sample questions" : "Example prompts"}</h3><p className="mt-1 text-sm text-muted-foreground">{isDemo ? "Choose a sample, then ask" : "Choose a prompt or write your own question."}</p><div className="mt-3 flex flex-col items-start gap-2">{examples.map((example) => <Button key={example} type="button" variant="outline" disabled={mutation.isPending} className="min-h-11 h-auto max-w-full whitespace-normal break-words py-2 text-left" onClick={() => { setQuestion(example); setValidation(undefined); mutation.reset(); }}>{example}</Button>)}</div></div>}
...
function EmptyResult({ isDemo, unavailable }: { isDemo: boolean; unavailable: boolean }) {
  return <ContentCard className="flex min-h-64 flex-col justify-center p-5 sm:p-6">
