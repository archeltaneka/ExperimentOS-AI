...
def test_public_causal_contracts_are_experimentos_owned() -> None:
    public_classes = [
        value for name in causal.__all__ if inspect.isclass(value := getattr(causal, name))
    ]
    assert public_classes
    assert all(
        item.__module__.startswith("packages.experiments.analysis") for item in public_classes
    )
