...
@pytest.mark.skipif(not os.environ.get("DATABASE_URL"), reason="DATABASE_URL is required")
def test_ask_endpoint_agent_workflow_uses_real_database_retrieval(
    tmp_path: Path,
    monkeypatch,
) -> None:
    from packages.db.session import create_async_session_factory, create_database_engine
    ...
    experiment_dir = tmp_path / "exp-api-ask-agent"
    experiment_dir.mkdir()
    (experiment_dir / "metadata.json").write_text(
        '{"experiment_id":"exp-api-ask-agent","name":"API Ask Agent Integration",'
        '"area":"payments","hypothesis":"test","owner":{"name":"Test Owner",'
        '"team":"Platform"},"status":"completed","start_date":"2026-01-01",'
        '"end_date":"2026-01-02","variants":[{"name":"control","description":"old"},'
        '{"name":"treatment","description":"new"}],"primary_metric":"conversion_rate",'
        '"secondary_metrics":["sample_size"],"imperfections":["synthetic"],'
        '"business_decision":"Roll out to clean markets."}',
        encoding="utf-8",
    )
    (experiment_dir / "metrics.csv").write_text(
        "experiment_id,metric_name,variant,value,unit,numerator,denominator,lift_vs_control,"
        "p_value,notes\n"
        "exp-api-ask-agent,conversion_rate,treatment,0.73,rate,73,100,0.08,0.02,"
        "Primary metric improved.\n",
        encoding="utf-8",
    )
    (experiment_dir / "report.md").write_text(
        "# API Ask Agent Integration\n\n"
        "## Recommendation\n"
        "Roll out to clean markets while monitoring wallet telemetry.\n",
        encoding="utf-8",
    )
    ...
    async def run_test() -> None:
        ...
        try:
            async with session_factory() as session:
                await session.execute(
                    delete(Experiment).where(Experiment.name == "API Ask Agent Integration")
                )
                await session.commit()
            ...
            async with session_factory() as session:
                experiment_id = await session.scalar(
                    select(Experiment.id).where(Experiment.name == "API Ask Agent Integration")
                )
            ...
            response = client.post(
                "/ask",
                json={
                    "question": "What happened in the API Ask Agent Integration experiment?",
                    "experiment_id": str(experiment_id),
                    "top_k": 3,
                },
            )
            ...
            assert retrieved_chunks
            assert any("wallet telemetry" in chunk["chunk_text"] for chunk in retrieved_chunks)
            assert "wallet telemetry" in response.json()["answer"]
            assert response.json()["agent_trace"][1]["node"] == "retrieval"
        finally:
            async with session_factory() as session:
                await session.execute(
                    delete(Experiment).where(Experiment.name == "API Ask Agent Integration")
