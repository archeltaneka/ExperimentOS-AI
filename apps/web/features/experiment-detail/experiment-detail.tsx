...
function Metrics({ experiment }: { experiment: ExperimentDetailRecord }) {
  ...
  return <section aria-labelledby="experiment-metrics"><h2 className="mb-3 text-xl font-semibold" id="experiment-metrics">Metrics</h2><div><div className="grid gap-4 md:grid-cols-2"><MetricCard metric={experiment.primaryMetric} primary />{supporting.map((metric) => <MetricCard key={metric.name} metric={metric} />)}</div><p className="mt-5 text-sm leading-6 text-muted-foreground">Observed differences are descriptive values in this record, not a causal impact estimate. Inferential statistics are unavailable unless an attached analysis result explicitly provides them.</p></div></section>;
}
...
export function ExperimentDetail({ experimentId, explorerSearch }: { experimentId: string; explorerSearch?: string }) {
  ...
  return <div className="space-y-8"><BackToExplorer explorerSearch={explorerSearch} /><Header experiment={experiment} /><DataSourceDisclosure /><Metrics experiment={experiment} /><div className="atlas-detail-columns"><div><DecisionSummary experiment={experiment} /><Overview experiment={experiment} /><ExperimentReport experiment={experiment} /></div><div><Evidence experiment={experiment} /><AnalysisReadinessSection readiness={experiment.analysisReadiness} /><BusinessImpact experiment={experiment} /></div></div><CapabilityMatrix capabilities={experiment.capabilities} /><RecordMetadata experiment={experiment} /></div>;
}
